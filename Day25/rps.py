...
def game_loop():
    ...
    while numTurnsCount > 0:
        ...
        try:
            ...
            userHandChoice = setUserHandName(input('Please enter [R]ock, [P]aper, [S]cissors, [E] to exit  ').upper())

            if userHandChoice == 'Exit':
               print('***Exiting***')
               break
        ...
        if userScore > 1 and numTurnsCount < 1:
            print(f'You won with a score of {userScore} out of {numTurns}')
        elif userScore < 2 and numTurnsCount < 1:
